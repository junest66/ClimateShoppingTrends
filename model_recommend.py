...
def mapper(data):
    print(data)
    """
    아래 mapping 세 개는 예시로, 전체 데이터 학습 시 mapping을 주석 내 mapping으로 바꿔야 함
    ...
    """
    city_mapping = {'전라북도': 0}
    area_mapping = {'전주시 덕진구': 1}
    main_mapping = {'Clear': 0}

    test = copy.deepcopy(data)
    test['city'] = city_mapping.get(test['city'], test['city']) 
    test['area'] = area_mapping.get(test['area'], test['area'])
    test['main'] = main_mapping.get(test['main'], test['main'])
    test['amount'] = 168000 # 평균값으로 바꿀 것
    test['count'] = 4.0 # 평균값으로 바꿀 것
...
def catboost(form_data):
    ...
    category_dict = {0: '공연', 1: '관람(내부)', 2: '관람(외부)', 3: '기타일반음식', 4: '레져업소', 5: '렌터카', 6: '숙박', 7: '양식', 8: '여행/교통', 9: '연료', 10: '오락위락시설', 11: '유통업', 12: '유흥주점', 13: '음료/주류', 14: '일식', 15: '자동차유지/정비', 16: '중식', 17: '취미교양용품', 18: '카페/디저트', 19: '한식'}
